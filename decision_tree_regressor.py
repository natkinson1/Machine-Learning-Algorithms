...
class DecisionTreeRegressor:
    ...
    def __init__(self, max_depth=1): #add more regularisation
    ...
    def best_split(self, X, y):
        ...
        for i in range(X.shape[1]):
            for value in np.unique(X[:, i]):
                ...
                error = prob * self._mse(y[true_index])\
                        + (1 - prob) * self._mse(y[false_index])
    ...
    def fit(self, X, y, max_depth=None):
        ...
        self.tree = DecisionNode(true_branch, false_branch, value, col) #This could be where the error lies, not sure what is going on here.
